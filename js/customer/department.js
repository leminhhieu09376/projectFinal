...
function loadDepartmentData() {
    fetch("http://localhost:3000/department/getActive",{
        method:"GET",
        headers: {
        ...
        response.json().then(data => ({
            ...
            if(result !== null) {
                for(let i=0; i<result.length; i++) {
                    ...
                    h3.style.color = "#333";
                    h3.appendChild(document.createTextNode(result[i].TenKhoa));
                    div.appendChild(h3);
                    $(li).append("<img src='../../images/customer_department.png' alt='' style='width:50px; height:50px; border-radius:50%'>")
                    li.appendChild(div);
...
function searchDepartment() {
    ...
    console.log(departmentSearch.value);
    if(departmentSearch.value !== "") {
        fetch("http://localhost:3000/department/searchActive/" + departmentSearch.value,{
            method:"GET",
            headers: {
                "Content-Type": "application/json"
            },
