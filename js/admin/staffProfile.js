...
function loadStaffProfileData() {
    const staffAccount = window.location.hash.substring(1)
    fetch("http://localhost:3000/staff/account/" + staffAccount,{
        method:"GET",
        headers: {
            "Content-Type": "application/json"
        },
        ...
        response.json().then(data => ({
            ...
            if(result !== null) {
                ...
                window.scroll(0, 0);
                $errorNotification.find("#error-message")
                    .text("Lấy chi tiết nhân viên thất bại, vui lòng thử lại sau");
                $errorNotification.fadeIn(500, "swing", function() {
                    setTimeout(function() {
                        $errorNotification.fadeOut(2000, "linear");
                    }, 2500);
    ...
    fetch("http://localhost:3000/staff",{
        ...
        response.json().then(data => ({
            ...
            if(res.data.status === "200") {
                ...
                window.scrollTo(0, 0);
                $errorNotification.find("#error-message")
                    .text("Cập nhật thông tin nhân viên thất bại, vui lòng thử lại sau");
                $errorNotification.fadeIn(500, "swing", function() {
                    setTimeout(function() {
                        $errorNotification.fadeOut(2000, "linear");
                    }, 2500);
