...
function loadDiseaseData() {
    diseaseId = window.location.hash.substring(1);
    if(diseaseId === "new") {
        loadNewId();
        return;
    }
    fetch("http://localhost:3000/disease/" + diseaseId,{
        ...
        response.json().then(data => ({
            ...
            let result = res.data.data;
            if(result !== null) {
                id.value = result.MaBenh;
                name.value = result.TenBenh;
            }else if(res.data.error !== null) {
                window.scrollTo(0, 0);
                $errorNotification.find("#error-message")
                    .text("Lấy chi tiết bệnh thất bại, vui lòng thử lại sau");
                $errorNotification.fadeIn(500, "swing", function() {
                    setTimeout(function() {
                        $errorNotification.fadeOut(2000, "linear");
                    }, 2500);
...
function loadNewId() {
    fetch("http://localhost:3000/disease/create/newId",{
        method:"GET",
        headers: {
            "Content-Type": "application/json"
        },
        ...
        response.json().then(data => ({
            ...
            if(result !== null) {
                ...
                window.scrollTo(0, 0);
                $errorNotification.find("#error-message")
                    .text("Tạo mã bệnh mới thất bại, vui lòng thử lại sau");
                $errorNotification.fadeIn(500, "swing", function() {
                    setTimeout(function() {
                        $errorNotification.fadeOut(2000, "linear");
                    }, 2500);
...
function checkDiseaseData() {
    ...
    fetch("http://localhost:3000/disease/checkDuplicateName",{
        method:"POST",
        headers: {
            "Content-Type": "application/json"
        },
        ...
        response.json().then(data => ({
            ...
            if(res.data.status === "200") {
                ...
                if(data.KQ === "N") {
                    ...
                    let alert = document.getElementById("name-alert");
                    alert.innerHTML = "* Tên bệnh này đã bị trùng";
                    alert.style.display = "block";
                }
    ...
    fetch("http://localhost:3000/disease",{
        ...
        response.json().then(data => ({
            ...
            if(res.data.status === "201") {
                window.location.href = "../../html/admin/disease.html#success";
            }else if(res.data.status === "500") {
                window.scrollTo(0, 0);
                $errorNotification.find("#error-message")
                    .text("Thêm bệnh thất bại, vui lòng thử lại sau");
                $errorNotification.fadeIn(500, "swing", function() {
                    setTimeout(function() {
                        $errorNotification.fadeOut(2000, "linear");
                    }, 2500);
        ...
        response.json().then(data => ({
            ...
            if(res.data.status === "200") {
                ...
                $successNotification.find("#success-message")
                    .text("Thông tin bệnh đã được cập nhật");
                ...
                window.scrollTo(0, 0);
                $errorNotification.find("#error-message")
                    .text("Cập nhật bệnh thất bại, vui lòng thử lại sau");
                $errorNotification.fadeIn(500, "swing", function() {
                    setTimeout(function() {
                        $errorNotification.fadeOut(2000, "linear");
                    }, 2500);
